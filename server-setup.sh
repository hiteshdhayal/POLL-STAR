...
DATABASE_URL=postgresql://user:password@localhost:5432/pollstar?connection_limit=20&pool_timeout=10
JWT_ACCESS_SECRET=your_access_secret_min_32_chars_here_abcdef
JWT_REFRESH_SECRET=your_refresh_secret_min_32_chars_here_xyz123
...
EOF
mkdir -p "$(dirname "server/.gitignore")"
...
cat > "server/package.json" << 'EOF'
{
  "name": "pollstar-server",
  "version": "1.0.0",
  "description": "PollStar backend - Express + TypeScript + Prisma",
  "main": "dist/index.js",
  "scripts": {
...
EOF
mkdir -p "$(dirname "server/prisma/schema.prisma")"
...
EOF
mkdir -p "$(dirname "server/src/config/env.ts")"
...
EOF
mkdir -p "$(dirname "server/src/config/prisma.ts")"
cat > "server/src/config/prisma.ts" << 'EOF'
...
EOF
mkdir -p "$(dirname "server/src/config/socket.ts")"
cat > "server/src/config/socket.ts" << 'EOF'
...
EOF
mkdir -p "$(dirname "server/src/index.ts")"
...
export default app;
EOF
mkdir -p "$(dirname "server/src/middleware/auth.middleware.ts")"
...
EOF
mkdir -p "$(dirname "server/src/middleware/error.middleware.ts")"
...
EOF
mkdir -p "$(dirname "server/src/middleware/rateLimiter.ts")"
...
EOF
mkdir -p "$(dirname "server/src/modules/analytics/analytics.controller.ts")"
cat > "server/src/modules/analytics/analytics.controller.ts" << 'EOF'
...
EOF
mkdir -p "$(dirname "server/src/modules/analytics/analytics.routes.ts")"
cat > "server/src/modules/analytics/analytics.routes.ts" << 'EOF'
...
EOF
mkdir -p "$(dirname "server/src/modules/analytics/analytics.service.ts")"
cat > "server/src/modules/analytics/analytics.service.ts" << 'EOF'
import { prisma } from '../../config/prisma';
import { AppError } from '../../utils/AppError';
...
EOF
mkdir -p "$(dirname "server/src/modules/auth/auth.controller.ts")"
...
EOF
mkdir -p "$(dirname "server/src/modules/auth/auth.routes.ts")"
...
export default router;
EOF
mkdir -p "$(dirname "server/src/modules/auth/auth.service.ts")"
...
EOF
mkdir -p "$(dirname "server/src/modules/auth/auth.validators.ts")"
cat > "server/src/modules/auth/auth.validators.ts" << 'EOF'
import { z } from 'zod';
...
EOF
mkdir -p "$(dirname "server/src/modules/polls/polls.controller.ts")"
cat > "server/src/modules/polls/polls.controller.ts" << 'EOF'
import { Request, Response } from 'express';
import { asyncHandler } from '../../utils/asyncHandler';
...
EOF
mkdir -p "$(dirname "server/src/modules/polls/polls.routes.ts")"
cat > "server/src/modules/polls/polls.routes.ts" << 'EOF'
import { Router } from 'express';
...
export default router;
EOF
mkdir -p "$(dirname "server/src/modules/polls/polls.service.ts")"
cat > "server/src/modules/polls/polls.service.ts" << 'EOF'
import { z } from 'zod';
import { prisma } from '../../config/prisma';
import { AppError } from '../../utils/AppError';
...
EOF
mkdir -p "$(dirname "server/src/modules/polls/polls.validators.ts")"
cat > "server/src/modules/polls/polls.validators.ts" << 'EOF'
import { z } from 'zod';
...
EOF
mkdir -p "$(dirname "server/src/modules/responses/responses.controller.ts")"
cat > "server/src/modules/responses/responses.controller.ts" << 'EOF'
import { Request, Response } from 'express';
import { asyncHandler } from '../../utils/asyncHandler';
...
EOF
mkdir -p "$(dirname "server/src/modules/responses/responses.routes.ts")"
cat > "server/src/modules/responses/responses.routes.ts" << 'EOF'
import { Router } from 'express';
...
export default router;
EOF
mkdir -p "$(dirname "server/src/modules/responses/responses.service.ts")"
cat > "server/src/modules/responses/responses.service.ts" << 'EOF'
import { prisma } from '../../config/prisma';
import { AppError } from '../../utils/AppError';
import { getIO } from '../../config/socket';
...
EOF
mkdir -p "$(dirname "server/src/modules/responses/responses.validators.ts")"
cat > "server/src/modules/responses/responses.validators.ts" << 'EOF'
import { z } from 'zod';
...
EOF
mkdir -p "$(dirname "server/src/utils/AppError.ts")"
cat > "server/src/utils/AppError.ts" << 'EOF'
...
EOF
mkdir -p "$(dirname "server/src/utils/asyncHandler.ts")"
cat > "server/src/utils/asyncHandler.ts" << 'EOF'
import { Request, Response, NextFunction } from 'express';
...
EOF
mkdir -p "$(dirname "server/src/utils/email.ts")"
cat > "server/src/utils/email.ts" << 'EOF'
...
export const sendPasswordResetEmail = async (to: string, name: string, token: string) => {
  ...
  await resend.emails.send({
    from: env.EMAIL_FROM,
    to,
    subject: 'Reset your PollStar password',
    html: `
      <div style="font-family: 'DM Sans', sans-serif; max-width: 600px; margin: 0 auto; background: #FAF7F2; padding: 48px 32px; border-radius: 8px;">
        <h1 style="font-family: Georgia, serif; font-size: 32px; color: #1A1A1A; margin-bottom: 8px;">POLL—STAR</h1>
        <p style="color: #6B6B6B; font-size: 12px; letter-spacing: 0.1em; text-transform: uppercase; margin-bottom: 32px;">SECURITY</p>
        <h2 style="font-family: Georgia, serif; font-size: 24px; color: #1A1A1A; margin-bottom: 16px;">Hello, ${name}.</h2>
        <p style="color: #1A1A1A; line-height: 1.6; margin-bottom: 32px;">
          We received a request to reset your password. Click the button below to choose a new password.
        </p>
        <a href="${resetUrl}" style="display:inline-block;padding:14px 32px;background:#C41E3A;color:white;text-decoration:none;font-size:12px;letter-spacing:0.1em;text-transform:uppercase;font-weight:600;">
          RESET PASSWORD →
        </a>
        <p style="color: #6B6B6B; font-size: 13px; margin-top: 32px;">This link expires in 1 hour. If you didn't request a password reset, you can safely ignore this email.</p>
        <hr style="border: none; border-top: 1px solid #E2DDD6; margin: 32px 0;" />
        <p style="color: #6B6B6B; font-size: 12px;">© 2025 PollStar. All rights reserved.</p>
      </div>
    `,
...
EOF
mkdir -p "$(dirname "server/src/utils/jwt.ts")"
...
EOF
mkdir -p "$(dirname "server/src/utils/tokens.ts")"
cat > "server/src/utils/tokens.ts" << 'EOF'
...
EOF
mkdir -p "$(dirname "server/tsconfig.json")"
